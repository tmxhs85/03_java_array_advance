...
public class ArrayEx47_정답 {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if(map[rY][rX] == 0) {
				// 공의 상하좌우에 벽이 없도록 코드를 수정하였다
				if ((map[rY-1][rX] != WALL) && (map[rY+1][rX] != WALL) && (map[rY][rX-1] != WALL) && (map[rY][rX+1] != WALL)) {
					map[rY][rX] = BALL;
					ballY = rY;
					ballX = rX;
					break;
		...
		while (true) {
			int rY = ran.nextInt(SIZE-2) + 1;
			int rX = ran.nextInt(SIZE-2) + 1;
			// 골대가 가장자리에 있지 않게 코드를 수정하였다
			
			if (map[rY][rX] == 0) {
				// 골대의 상하좌우에 벽이 없도록 코드를 수정하였다
				if ((map[rY-1][rX] != WALL) && (map[rY+1][rX] != WALL) && (map[rY][rX-1] != WALL) && (map[rY][rX+1] != WALL)) {					
					map[rY][rX] = GOAL;
					goalY = rY;
					goalX = rX;
		...
		// 공과 골대가 가장자리에 있지 않고, 상하좌우에 벽이 없도록 코드를 짜더라도
		// 애초에 깰 수 없는 단계가 생성되는 경우가 있다
		// 이를 위한 예외처리를 생각해볼 필요가 있다
